...
def main():
    ...
    # We fade:
    # - Left/right edges (width 140 pixels)
    # - Top edge (height 65 pixels) to remove the sharp horizontal cut to the galaxy!
    ...
    for y in range(inner_h):
        for x in range(inner_w):
            # Calculate horizontal fade
            if x < fade_w:
                alpha_x = int((x / float(fade_w)) * 255)
            elif x > inner_w - 1 - fade_w:
                alpha_x = int(((inner_w - 1 - x) / float(fade_w)) * 255)
            else:
                alpha_x = 255
                
            # Calculate vertical fade at the top edge of the scene
            if y < fade_h:
                alpha_y = int((y / float(fade_h)) * 255)
            else:
                alpha_y = 255
                
            # The final alpha is the minimum of both fades, creating a seamless vignette boundary
            mask_draw.point((x, y), fill=min(alpha_x, alpha_y))

    # 7. Paste the scene at x=156, y=300 (bottom section)
    # Thanks to the vertical alpha fade, the top transition blends seamlessly with the background sky!
    ...
    logo_w = 1020
    logo_h = int(logo.height * (logo_w / float(logo.width)))
